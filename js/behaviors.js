$(document).ready(function() {
  $('.ignore').bind('click', function() {
    ...
    $photo.append('<div class="status"><p>Ignoring this photo...</p><img src="img/indicator.gif" width="64" height="64" /></div>');
...
$('.favor').bind('click', function() {
  ...
  var $photo = $this.parent().parent();
  $photo.find('img').animate({ opacity: 0.25 }, 1000);
  $photo.append('<div class="status"><p>Favoring this photo...</p><img src="img/indicator.gif" width="64" height="64" /></div>');
  var $href = $this.attr('href') + '&mode=ajax';
  $.get($href, function(data) {
    if (data == '1') {
      $photo.hide('slow', function() { $(this).remove(); });
